...
## --------------------------------------------------------------------------------------------------------------------------------------------------------
if(SEL_BLUP){
  perf_Bras <- Create_Perf_Bras(Output_Q, Output_TQ, erase_Perf_T1_Q=ERASE_PERF_T1_Q_ALT, erase_Perf_T2_Q=ERASE_PERF_T2_Q_ALT, erase_Perf_T1_TQ=ERASE_PERF_T1_TQ_ALT, erase_Perf_T2_TQ=ERASE_PERF_T2_TQ_ALT,
   year_erase_Perf_T1_Q=YEAR_ERASE_PERF_T1_Q_ALT, year_erase_Perf_T2_Q=YEAR_ERASE_PERF_T2_Q_ALT, year_erase_Perf_T1_TQ=YEAR_ERASE_PERF_T1_TQ_ALT, year_erase_Perf_T2_TQ=YEAR_ERASE_PERF_T2_TQ_ALT)
}
...
## --------------------------------------------------------------------------------------------------------------------------------------------------------
if(SEL_BLUP){
  CandidateBQ_n_DPQ_n <- Retrieve_BLUP_sol(WD_PATH_BLUP, perf_Bras_joinID, SINGLE_TRAIT, CandidateBQ_n=WQ_n, CandidateDPQ_n=WTQ_n_save_one)
  
  WQ_n <- CandidateBQ_n_DPQ_n[[1]]
  WTQ_n_save_one <- CandidateBQ_n_DPQ_n[[2]] #first winter surviving TQs
}
...
if(SEL_BLUP){
  ...
  W2_TQ_n_save_one <- Join_EBVs_Q_n(Table_to_update_n=W2_TQ_n_save_one, CandidateDPQ_n=WTQ_n_save_one, SINGLE_TRAIT)
  ...
  if(SINGLE_TRAIT == FALSE){
    Output_Q <- Output_Q %>% left_join(WQ_n %>% select(ID,
                                       ...
                                       by="ID")
    Output_TQ <- Output_TQ %>% left_join(WTQ_n_save_one %>% select(ID,
                                                                   EBV_d1_Q, EBV_m1_Q, EBV_d2_Q, EBV_m2_Q,
                                                                   EBV_d1_W, EBV_m1_W, EBV_d2_W, EBV_m2_W,
                                                                   SE_EBV_d1_Q, SE_EBV_m1_Q, SE_EBV_d2_Q, SE_EBV_m2_Q,
                                                                   SE_EBV_d1_W, SE_EBV_m1_W, SE_EBV_d2_W, SE_EBV_m2_W,
...
if(SEL_BLUP){
  if(STOP_IF_CONV_FAILED){
    conv_crit_failed <- unique(as.numeric(WQ_n$Conv_crit))
    round_failed <- unique(as.numeric(WQ_n$Conv_round))
    if(conv_crit_failed > 1e-11 | conv_crit_failed == 0){
      write(
        paste0("BLUP estimation did not reach the convergence criterion in scenario Alt at year ",
...
## --------------------------------------------------------------------------------------------------------------------------------------------------------
BQ_n <- Select_BQ(WQ_n, NB_BQ, Q_SELECTION_CRITERION)
...
## ---- eval=F---------------------------------------------------------------------------------------------------------------------------------------------
## WQ_n_save_one <- WQ_n # Q_n_save_one will store queens of the previous year
...
if(pedigree_Pim){
  ...
  pedigree <- new_pedigree_female(pedigree, New_virgin_Qs=WQ_n, NS_Q_or_TQ=NS_Q, birth_year_dams=(year-1), NB_D_FECUNDATING, ID_1b=UNKNOWN_1b)
}


## --------------------------------------------------------------------------------------------------------------------------------------------------------
ped_A <- Create_ped_A_Q_n(W_indiv_n=WQ_n, Dams=BQ_n, Hap_Sires=D_n, index_dams=NB_Q_TQ_AND_D, index_D=NB_Q_TQ_AND_D+NB_W1_Q+NB_W1_TQ, index_indiv=(NB_W1_Q+NB_W1_TQ+NB_D)*2 ) # dams, born in year LAST_YEAR_BURN_IN, come after WQs, WTQs and Ds of LAST_YEAR_BURN_IN -2


## --------------------------------------------------------------------------------------------------------------------------------------------------------
A <- Extend_A_with_Q_n(A, ped_A, NB_indiv_to_truncate=0, NB_W1_indiv=NB_W1_Q) # We keep TQs born in year LAST_YEAR_BURN_IN -2  in A because we will use them as DPQ this year
...
## --------------------------------------------------------------------------------------------------------------------------------------------------------
ped_A <- Create_ped_A_Q_n(W_indiv_n=WTQ_n, Dams=BQ_n, Hap_Sires=D_n, index_dams=NB_Q_TQ_AND_D, index_D=NB_Q_TQ_AND_D+NB_W1_Q+NB_W1_TQ, index_indiv=(NB_Q_TQ_AND_D)*2 + NB_W1_Q)
...
if(PAT_LINE_SELECTION==TRUE){
  Q_of_PS_n <- Select_PS_from_WTQ(W1_CandidateDPQ=WTQ_n_save_two, W2_CandidateDPQ=W2_TQ_n_save_one, NB_PGM, NB_BQ, Criterion=PGM_SELECTION_CRITERION)
...
## --------------------------------------------------------------------------------------------------------------------------------------------------------
if(PAT_LINE_SELECTION==TRUE){
  DPQ_n <- Create_DPQ_n_blanck(W2_CandidateDPQ=W2_TQ_n_save_one, NB_PGM, NB_DPQ_PER_PS)
}


## --------------------------------------------------------------------------------------------------------------------------------------------------------
if(PAT_LINE_SELECTION==TRUE){
  DPQ_n <- Fill_DPQ_n(Q_of_PS_n, W2_CandidateDPQ=W2_TQ_n_save_one, NB_PGM, NB_DPQ_PER_PS, DPQ_n, Criterion=DPQ_SELECTION_CRITERION)
...
if(PAT_LINE_SELECTION==FALSE){
  DPQ_n <- Select_DPQs(W1_CandidateDPQ=WTQ_n_save_two, W2_CandidateDPQ=W2_TQ_n_save_one, NB_TQ, NB_DPQ, Criterion=DPQ_SELECTION_CRITERION)
...
if(pedigree_Pim){
  ...
  pedigree <- Add_PGM_mated_to_pedigree(pedigree, PS_or_TQ_to_WQ_n, NS=NS_Q, MATING_SCALE) # For this transition part, we don't have DPQ_9 (not produced in BurnIn phase), so we use the latest DPQs available: DPQ_8. After that, DPQs will always be from year-2
}
...
## --------------------------------------------------------------------------------------------------------------------------------------------------------
ped_A <- Create_ped_A_D_n(WTQ_with_DPQs=WTQ_n_save_two, D_n,
                          A,
                          DPQ_index=NB_W1_Q, 
                          NB_D)
...
## --------------------------------------------------------------------------------------------------------------------------------------------------------
W2_TQ_n_save_one <- W2_TQ_n # These queens will serve as DPQ, so we save their annual table
...
## --------------------------------------------------------------------------------------------------------------------------------------------------------
if(SEL_BLUP){
  Output_Q <- Join_EBVs_to_Output(Output_Q_or_TQ=Output_Q, Candidates_n=WQ_n, SINGLE_TRAIT)
  
  Output_TQ <- Join_EBVs_to_Output(Output_Q_or_TQ=Output_TQ, Candidates_n=WTQ_n_save_one, SINGLE_TRAIT)
}
...
## --------------------------------------------------------------------------------------------------------------------------------------------------------
A <- Extend_A_with_Q_n(A, ped_A, NB_indiv_to_truncate=NB_Q_TQ_AND_D, NB_W1_indiv=NB_W1_Q) # We keep TQs born in year LAST_YEAR_BURN_IN -2  in A because we will use them as DPQ this year
...
## --------------------------------------------------------------------------------------------------------------------------------------------------------
for (year in seq(from=year+1, to=(LAST_YEAR), by=1)){
  tn_start <- proc.time()
...
# Output_Q
# initialization
Output_Q$Scheme_scenario[is.na(Output_Q$Scheme_scenario)] <- "AltScen"

## --------------------------------------------------------------------------------------------------------------------------------------------------------
# Output_TQ: only works for Q_x_DPQ mating
# initialization
Output_TQ$Scheme_scenario[is.na(Output_TQ$Scheme_scenario)] <- "AltScen"
...
fwrite(Output_Q, file=paste0(WD_PATH_OUTPUT,"/Output_Q_AltScen.csv"), sep = ";", dec = ".", row.names = FALSE, col.names = TRUE)

fwrite(Output_TQ, file=paste0(WD_PATH_OUTPUT,"/Output_TQ_AltScen.csv"), sep = ";", dec = ".", row.names = FALSE, col.names = TRUE)
...
# save.image(paste0("AltSchen_", year, ".RData"))
