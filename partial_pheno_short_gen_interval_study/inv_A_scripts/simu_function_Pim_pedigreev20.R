...
Pim_pedigree <- function(WD_PATH_BLUP){

setwd(WD_PATH_BLUP)#and set the working directory to that location


t1 <- proc.time()#initiating counter to get running time
    ...
    if(nodamss==0){
      ...
      if(jd==0){
        ...
        if(msinglesires==1){
          ...
          write.table(ssinglesires,file="singlesires.txt",row.names=FALSE,col.names=FALSE, quote = FALSE) # quotes around char chains in output if last option is not set
        }
        ################################################################################
        ...
        write.table(identt,file="ident.txt",row.names=FALSE,col.names=FALSE, quote = T)
        write.table(hh,file="pedigree.txt",row.names=FALSE,col.names=FALSE, na="0")
...
t2 <- proc.time()
running_time_total <- t2-t1
print(paste0("pedigree took ", running_time_total[3]/60, " min to run with codetest=", codetest))
