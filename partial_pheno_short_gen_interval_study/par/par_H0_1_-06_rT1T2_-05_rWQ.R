...
Q_SELECTION_CRITERION_INI <- "0*(Perf_T1/sd(Perf_T1))+1*(Perf_T2/sd(Perf_T2))" # 0.5*(Perf_T1/sd(Perf_T1))+0.5*(Perf_T2/sd(Perf_T2)) Perf_T1/sd(Perf_T1) + Perf_T2/sd(Perf_T2) "Perf_T1" "Perf_T2" "Random"  etc. or simply Random
PGM_SELECTION_CRITERION_INI <- "0*(Perf_T1/sd(Perf_T1))+1*(Perf_T2/sd(Perf_T2))" # "Perf_T1" "Perf_T2" "Random"  etc. or simply Random to select 4as
DPQ_SELECTION_CRITERION_INI <- "0*(Perf_T1/sd(Perf_T1))+1*(Perf_T2/sd(Perf_T2))" # "Perf_T1" "Perf_T2" "Random"  etc. or simply Random
# Selection criterion for selection (after burn in, with possibly BLUP estimation)
Q_SELECTION_CRITERION <- "0*(EBV_d1_W+EBV_m1_W)+1*(EBV_d2_W+EBV_m2_W)" #IF SINGLE TRAIT, CANNOT USE EBVs OF T2 0.5*(EBV_d1_W+EBV_m1_W)+0.5*(EBV_d2_W+EBV_m2_W) "EBV_d1_W+EBV_m1_W+EBV_d2_W+EBV_m2_W" "Perf" "Random" calculus using Blup_d_col, Blup_m_col, Perf etc. or simply Random
PGM_SELECTION_CRITERION <- "0*(EBV_d1_W+EBV_m1_W)+1*(EBV_d2_W+EBV_m2_W)" # Perf_T1 + Perf_T2 calculus using EBV_d1_Q, EBV_m1_Q, EBV_d2_Q,EBV_m2_Q, Perf etc. or simply Random to select 4as
DPQ_SELECTION_CRITERION <- "0*(EBV_d1_Q+EBV_m1_Q)+1*(EBV_d2_Q+EBV_m2_Q)" #Perf_T2 calculus using EBV_d1_Q, EBV_m1_Q, EBV_d2_Q,EBV_m2_Q, Perf etc. or simply Random
...
# Genetic covariance among each of both traits
COV_A1_D_M <- -7.07107
COV_A2_D_M <- -7.07107 #direct-maternal genetic covariance
# Genetic covariance between each of both traits
COV_A1_A2_D <- -12 # 0 -6 -12
COV_A1_A2_M <- -6 # 0 -3 -6
COV_A1_D_A2_M <- 4.24264 # direct-maternal genetic covariance between traits
COV_A1_M_A2_D <- 4.24264 # direct-maternal genetic covariance between traits
